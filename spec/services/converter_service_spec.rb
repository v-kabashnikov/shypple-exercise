...
RSpec.describe ConverterService, type: :service do
  let(:data) { JSON.parse(File.read('response.json')) }
  let(:exchange_rates) { data['exchange_rates'] }
  let(:sailings) { data['sailings'] }
  let(:rates) { data['rates'] }

  subject { described_class.new(exchange_rates) }

  describe '#to_usd' do
    it 'converts rates to USD' do
      converted_rates = subject.to_usd(rates, sailings)

      expect(converted_rates).to match_array([
